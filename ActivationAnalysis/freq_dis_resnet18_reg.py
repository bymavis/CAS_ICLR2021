...
from utils import makedirs, create_logger, tensor2cuda, numpy2cuda, evaluate, save_model, load_model, LabelDict
import copy
import matplotlib
import argparse
matplotlib.use('Agg')
...
parser = argparse.ArgumentParser(description='Frequency Analysis for CAS model.')
...
parser.add_argument('--checkpoint', default='./output/resnet18_cas.pth',
...
model_path_adv = args.checkpoint


model_adv = ResNet18_REG(num_classes=10)
model_std = ResNet18_REG(num_classes=10)
model_clip = ResNet18_REG(num_classes=10)

load_model(model_adv, model_path_adv)
load_model(model_std, model_path_adv)
load_model(model_clip, model_path_adv)
...
def _pgd_whitebox(model,
    ...
    for _ in range(num_steps):
        ...
        with torch.enable_grad():
            output, extra_output = model(X_pgd, _eval=True)
...
#########################################################################################
    # register handler
    relu_index = 0
    model_adv.layer4[1].relu.register_forward_hook(get_features_hook)
    model_clip.layer4[1].relu.register_forward_hook(get_features_hook_std)
...
###########################################################################################
    ...
    for data, label in tqdm(te_loader):
        ...
        adv_data1 = _pgd_whitebox(model_std, data, label, beta=2)


        output1, _ = model_adv(adv_data1, _eval=True)
        output2, _ = model_std(adv_data1, _eval=True)
        output3, _ = model_clip(data, _eval=True)
...
#################################################################################
    ...
    np.save('./Frequency/cifar10_cas_layer4_class0.npy', res)
    ...
    np.save('./Magnitude/cifar10_cas_layer4_class0.npy', res)
