...
#
# class BasicBlock_Channel_Reg(nn.Module):
#     def __init__(self, in_planes, out_planes, stride, dropRate=0.0):
#         super(BasicBlock_Channel_Reg, self).__init__()
#         self.bn1 = nn.BatchNorm2d(in_planes)
#         self.relu1 = nn.ReLU(inplace=True)
#         self.conv1 = nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,
#                                padding=1, bias=False)
...
#         return self.layer(x)
#
# class NetworkBlock_Channel_Reg(nn.Module):
#     def __init__(self, nb_layers, in_planes, out_planes, block, stride, dropRate=0.0):
#         super(NetworkBlock_Channel_Reg, self).__init__()
#         self.nb_layers = nb_layers
#         self.layer = self._make_layer(BasicBlock_Channel_Reg, in_planes, out_planes, nb_layers, stride, dropRate)
#
#     def _make_layer(self, block, in_planes, out_planes, nb_layers, stride, dropRate):
#         layers = []
#         for i in range(int(nb_layers)):
...
#         #     layers.append(BasicBlock_Channel_Reg(i == 0 and in_planes or out_planes, out_planes, i == 0 and stride or 1, dropRate))
#         # for i in range(int(nb_layers)-2):
#         #     layers.append(BasicBlock(i == 0 and in_planes or out_planes, out_planes, i == 0 and stride or 1, dropRate))
#         # for i in range(int(nb_layers)):
#         #     layers.append(block(i == 0 and in_planes or out_planes, out_planes, i == 0 and stride or 1, dropRate))
...
#         for i in range(0, int(self.nb_layers)):
...
#         # 3rd block
#         self.block3 = NetworkBlock_Channel_Reg(n, nChannels[2], nChannels[3], block, 2, dropRate)
...
class NetworkBlock_CAS(nn.Module):
    def __init__(self, nb_layers, in_planes, out_planes, block, stride, dropRate=0.0):
        ...
        self.nb_layers = nb_layers
        self.layer = self._make_layer(BasicBlock_CAS, in_planes, out_planes, nb_layers, stride, dropRate)
    ...
    def _make_layer(self, block, in_planes, out_planes, nb_layers, stride, dropRate):
        ...
        for i in range(int(nb_layers)-1, int(nb_layers)):
            layers.append(BasicBlock_CAS(i == 0 and in_planes or out_planes, out_planes, i == 0 and stride or 1, dropRate))
        return nn.ModuleList(layers)
...
class WideResNet(nn.Module):
    def __init__(self, depth=34, num_classes=10, widen_factor=10, dropRate=0.0):
        ...
        self.block2 = NetworkBlock(n, nChannels[1], nChannels[2], block, 2, dropRate)
        # 3rd block
        self.block3 = NetworkBlock_CAS(n, nChannels[2], nChannels[3], block, 2, dropRate)
        # global average pooling and classifier
        self.bn1 = nn.BatchNorm2d(nChannels[3])
        self.relu = nn.ReLU(inplace=True)
        self.fc = nn.Linear(nChannels[3], num_classes)
